...
flz::RWMutex m_mutex;
//name -> session
std::map<std::string, flz::http::WSSession::ptr> m_sessions;

bool session_exists(const std::string& id) {
    FLZ_LOG_INFO(g_logger) << "session_exists id=" << id;
    flz::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(id);
    return it != m_sessions.end();
}

void session_add(const std::string& id, flz::http::WSSession::ptr session) {
    FLZ_LOG_INFO(g_logger) << "session_add id=" << id;
    flz::RWMutex::WriteLock lock(m_mutex);
    m_sessions[id] = session;
}

void session_del(const std::string& id) {
    FLZ_LOG_INFO(g_logger) << "session_add del=" << id;
    flz::RWMutex::WriteLock lock(m_mutex);
    m_sessions.erase(id);
}

int32_t SendMessage(flz::http::WSSession::ptr session
                    , ChatMessage::ptr msg) {
    FLZ_LOG_INFO(g_logger) << msg->toString() << " - " << session;
    return session->sendMessage(msg->toString()) > 0 ? 0: 1;
}

void session_notify(ChatMessage::ptr msg, flz::http::WSSession::ptr session = nullptr) {
    flz::RWMutex::ReadLock lock(m_mutex);
...
int32_t ChatWSServlet::onConnect(flz::http::HttpRequest::ptr header
    ...
    FLZ_LOG_INFO(g_logger) << "onConnect " << session;
    return 0;
}
...
int32_t ChatWSServlet::onClose(flz::http::HttpRequest::ptr header
    ...
    FLZ_LOG_INFO(g_logger) << "onClose " << session << " id=" << id;
    if(!id.empty()) {
        session_del(id);
        ChatMessage::ptr nty(new ChatMessage);
        nty->set("type", "user_leave");
        nty->set("time", flz::Time2Str());
        nty->set("name", id);
        session_notify(nty);
    }
    return 0;
...
int32_t ChatWSServlet::handle(flz::http::HttpRequest::ptr header
                           ...
                           ,flz::http::WSSession::ptr session) {
   	FLZ_LOG_INFO(g_logger) << "handle " << session
